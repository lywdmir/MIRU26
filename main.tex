...
まず，大域的な幾何構造の観点から，提案計量下の測地線が多様体に沿って推移することを合成データで確認し，さらに画像や動画フレームの補間において知覚的に自然な遷移が実現されることを確認した．
また，局所的な幾何構造の観点からは，本計量に基づくガイダンス補正が生成品質を向上させ，その効果が追加コストなく蒸留モデルへも継承されることを示した．

% 拡散モデルは，高品質かつ多様なデータを生成できる深層生成モデル（DGM）の一種である~\cite{Ho2020,Song2021a,Rombach2022}．
% DGM の理解と改善には，実世界のデータが高次元空間中の低次元多様体近傍に集中するという多様体仮説が重要な役割を果たす~\cite{Bengio2012,Fefferman2016}．
% この仮定のもとでは，DGMはデータ分布だけでなく，その背後にある多様体構造を暗黙的または明示的に学習していると解釈される~\cite{Loaiza-ganem2024}．
...
% 先行研究では密度や多様体への方向に基づく計量が提案されているが~\cite{Yu2025,Azeglio2025}（図~\ref{fig:illust}），測地線が高密度領域へ過度に引き寄せられ，過平滑化された画像を生成する傾向がある~\cite{Karczewski2025a}．
