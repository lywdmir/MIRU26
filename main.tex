...
拡散モデルは，高忠実度かつ多様なコンテンツを生成する能力を示した深層生成モデル（DGM）の一種である~\cite{Ho2020,Song2021a,Rombach2022}．
DGMを理解・改善するための重要な理論的視点として\emph{多様体仮説}がある．
これは，実世界のデータ（例えば画像）が高次元データ空間に埋め込まれた低次元多様体の周囲に集中しているという仮定である~\cite{Bengio2012,Fefferman2016}．
この仮定のもとでは，DGMはデータ分布だけでなく，その基礎となる多様体を陰的または陽的に学習していると理解される~\cite{Loaiza-ganem2024}．

VAE~\cite{Kingma2014}やGAN~\cite{Goodfellow2014}はデータ多様体の陽的なパラメータ化として機能する低次元潜在空間を持ち，デコーダを通じたデータ空間計量のプルバックによってリーマン計量を定義できる~\cite{Gruffaz2025}．
例えば，測地線に沿った潜在空間の移動は，データの本質的な幾何構造に忠実な補間を与える~\cite{Shao2017,Arvanitidis2018,Chen2018,Arvanitidis2021}．
一方，拡散モデルはそのような低次元潜在空間を持たない．
ノイズ空間はデータ空間と同次元であり，意味のあるプルバック計量を定義することは非自明である．

先行研究では，ノイズサンプルの密度や多様体への方向に基づいて拡散モデルのリーマン計量を定義する試みがある~\cite{Yu2025,Azeglio2025}（\cref{fig:illust}）．
しかし，これらの計量のもとでの測地線はデータ分布の中心に過度に近づき，過平滑化された生成結果をもたらす可能性がある~\cite{Karczewski2025a}．
根本的な問題は，密度や方向はサンプルが\emph{どこに}集中するかを示すものの，環境空間において多様体が\emph{どのように}配向されているかを示さない点にある．
これは自然な問いを提起する：\emph{密度ではなくデータ多様体の本質的な幾何を直接反映する，拡散モデルのリーマン計量を定義できるか？}

本稿では，この問いに肯定的に答える．
我々の鍵となる観察は，スコア関数のヤコビアンがこの幾何をすでに符号化しているという点である：そのスペクトル構造はデータ多様体の接線方向と法線方向を分離する~\cite{Stanczuk2024,Ventura2025}（\cref{fig:concept}）．
この知見に基づき，スコア関数を通じたユークリッド計量のプルバックとして定義されるノイズ空間上のリーマン計量を提案する．
この計量は法線方向への移動に高いコストを，接線方向には低いコストを割り当て，パスをデータ多様体に接する方向に保つよう促す．
事前学習済みスコア関数のヤコビアンのみから構築されるため，追加の学習やアーキテクチャの変更を必要としない．

本稿の貢献は，この計量そのものと，それが提供するノイズ空間の接線・法線分解の観点にある．
提案計量の妥当性を二つの補完的な角度から検証する．
\textbf{(i)~補間による大域的幾何}（\cref{sec:synthetic,sec:image_interp,sec:video_interp}）：
提案計量のもとでの測地線は多様体上またはその接線方向に沿って推移し，知覚的に自然な遷移を生む．
合成データ，画像，動画フレームデータセットでの実験がこれを確認する．
\textbf{(ii)~ガイダンス補正による局所的幾何}（\cref{sec:guidance}）：
提案計量による接線・法線分解は，Classifier-Free Guidance（CFG）~\cite{Ho2021}がサンプルを多様体から逸脱させることを防ぐ．
このガイダンス補正はCLIPスコアを維持しながらFIDを一貫して改善し，蒸留モデルにも追加のオーバーヘッドなく転用できる．

\section{関連研究}

\subsection{深層生成モデルのリーマン幾何}
VAEやGANの潜在空間はデータ多様体をパラメータ化しており，線形移動による意味属性の編集が広く用いられてきた~\cite{Goetschalckx2019,Harkonen2020,Shen2020}．
しかし実世界のデータ分布は偏りがあるため，非線形アプローチが動機付けられる~\cite{Jahanian2020,Aoshima2023}．
こうした非線形性を体系的に扱う方法として，潜在空間へのリーマン計量の導入がある~\cite{Gruffaz2025}．
追加ネットワークで計量を学習する手法~\cite{Yang2018,Arvanitidis2022}は元の多様体構造を上書きする恐れがある．
一方，事前学習済みモデルのデコーダを通じたプルバック計量を用いる学習不要な手法もある~\cite{Shao2017,Chen2018,Arvanitidis2018,Arvanitidis2021}．
本研究はこの後者の学習不要なアプローチを踏襲しつつ，ノイズ空間が低次元でない拡散モデルへ拡張する．

\subsection{拡散モデルにおけるデータ多様体}
拡散モデルは，スコア関数 $s_\theta(x_t, t)$ を通じて $t=T$ から $t=0$ へ段階的にノイズ除去を行い，データ分布を学習する~\cite{Ho2020,Song2021a,Song2021b}．
$t>0$ におけるノイズサンプルの空間はしばしば\emph{ノイズ空間}と呼ばれる．
拡散モデルはこの除去過程を通じてデータ多様体を暗黙的に学習することが示されている~\cite{Pidstrigach2022,Tang2024}．
...
\subsection{ガイダンス手法}
Classifier-Free Guidance（CFG）~\cite{Ho2021} はテキスト条件への忠実度を高める一方，学習済みデータ多様体を歪め，視覚的に不自然な生成物をもたらすことが知られている．
この問題を緩和するために，CFG++~\cite{Chung2025} は再ノイズ化ステップに無条件スコアを用い，TCFG~\cite{Kwon2025} はSVDにより無条件スコアを条件付きスコアと共有する部分空間へ射影する．
...
\subsubsection{定義．}
スコア関数を通じてデータ多様体の局所幾何を捉えるノイズ空間上のリーマン計量を定義する．
$x_t \in \mathbb{R}^D$ をノイズ空間の点，$v, w \in T_{x_t}\mathbb{R}^D$ を接ベクトル，$s_\theta(x_t, t)$ を事前学習済み拡散モデルのスコア関数とする．
時刻 $t$ における提案計量を以下のように定義する：
\begin{equation}
    \label{eq:jacobian_metric}
    g_{x_t}(v, w) \coloneqq \langle J_{x_t} v,\, J_{x_t} w \rangle = v^\top J_{x_t}^\top J_{x_t} w = v^\top G_{x_t} w,
\end{equation}
ここで $J_{x_t} = \nabla_{x_t} s_\theta(x_t, t)$ はスコア関数のヤコビアン，$G_{x_t} = J_{x_t}^\top J_{x_t}$ は計量の行列表現である．
この計量はスコア空間 $\mathbb{R}^D$ 上のユークリッド計量 $I$ のスコア関数 $s_\theta$ を通じたプルバック $s_\theta^* I$ である．
...
\begin{equation}
    ...
    E[\gamma] = \frac{1}{2}\int_0^1 \|J_{\gamma(u)} \gamma'(u)\|_2^2\, \mathrm{d}u
...
本稿の主要な貢献は \cref{eq:jacobian_metric} で定義したリーマン計量である．
その妥当性を，補間による大域的幾何と，ガイダンス補正による局所的幾何の二つの角度から検証する．

\subsection{合成2Dデータ}
\label{sec:synthetic}

C字形分布に対して DDPM~\cite{Ho2020}（$T=50$）を学習し，$\tau=1$ における補間を評価した（\cref{fig:illust}）．
...
\begin{figure*}[p]
    ...
    \begin{tabular}{rc}
        \raisebox{2.9mm}{\textcolor{BLUE}{LERP}}      & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_lerp.jpg}}      \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{GREEN}{SLERP}}    & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_slerp.jpg}}     \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{PURPLE}{NAO}}     & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_nao.jpg}}       \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{PINK}{NoiseDiff}} & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_noisediff.jpg}} \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{ORANGE}{GeoDiff}} & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_geodiff.jpg}}   \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{GRAY}{FIM-based}} & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_fim.jpg}}       \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{\textcolor{BLACK}{Ours}}     & {\includegraphics[width=15cm]{assets/interpolation/image/vangogh_ours.jpg}}      \\ \addlinespace[1.5pt]
    ...
    \renewcommand{\arraystretch}{0}
    \begin{tabular}{rc}
        \raisebox{2.9mm}{MB (A)}    & {\includegraphics[width=10.4cm]{assets/interpolation/image/chair_ours.jpg}}    \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{CA}       & {\includegraphics[width=10.4cm]{assets/interpolation/image/female13_ours.jpg}} \\ \addlinespace[1.5pt]
        \raisebox{2.9mm}{AF (Cat)} & {\includegraphics[width=10.4cm]{assets/interpolation/image/cat6_ours.jpg}}     \\ \addlinespace[1.5pt]
                                   & \includegraphics[scale=0.946]{assets/interpolation/imagelabel.drawio-crop.pdf}                 \\ \addlinespace[1.5pt]
    \end{tabular}
    \caption{Qualitative examples of interpolated image sequences by our method.
        See also \cref{fig:results_qualitative_appendix} in \cref{appendix:additional_results}.
    }
    \label{fig:results_qualitative2}
\end{figure*}
...
\subsubsection{貢献のまとめ．}
本稿では，スコア関数のヤコビアンから導かれる拡散モデルのノイズ空間上のリーマン計量を提案した．
主要な貢献を以下に整理する．

\textbf{(1)~計量の定義と理論的裏付け．}
スコア関数のプルバック計量 $G_{x_t} = J_{x_t}^\top J_{x_t}$ を提案し，そのスペクトル構造がデータ多様体の接線・法線空間を分離することを命題として示した．
既存の密度ベース計量~\cite{Yu2025} や FIM ベース計量~\cite{Azeglio2025} が1方向のみ（または密度のみ）を捉えるのに対し，本計量はヤコビアンの全スペクトル構造を利用する．

\textbf{(2)~学習不要な実装．}
追加の学習やアーキテクチャ変更を一切必要とせず，事前学習済みスコア関数のヤコビアン・ベクトル積（有限差分近似）のみから計算できる．
任意の拡散モデルに適用可能である．

\textbf{(3)~大域的幾何の検証：測地線補間．}
合成2Dデータ・4種の画像データセット・3種の動画ベンチマークにわたる実験で，提案測地線が多様体に沿って推移し，PPL・PDV・FID・RE および MSE・LPIPS のすべての指標で先行手法を上回ることを確認した．

\textbf{(4)~局所的幾何の検証：ガイダンス補正．}
提案計量の接線・法線分解を CFG に適用することで，CLIPスコアを維持しながら FID を一貫して改善する．
また，蒸留（LCM）への適用により，4ステップの高速推論においても品質向上を学生モデルに継承できることを示した．

\subsubsection{将来展望．}
本計量は補間とガイダンス補正の枠を超えた広範な応用の幾何的基盤となると期待される．
具体的には，（i）ノイズ空間における意味的編集や属性操作，（ii）接線空間に基づくノイズ空間クラスタリング，（iii）フローマッチングモデルや EBM への拡張，（iv）最近のアーキテクチャ（Transformer ベースの拡散モデル等）での計量の挙動の検証，などが今後の重要な課題である．
また，ヤコビアンの計算コスト削減（低ランク近似や確率的推定）も実用上の課題として残る．
